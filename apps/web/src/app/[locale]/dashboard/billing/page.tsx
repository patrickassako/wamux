...
export default function BillingPage() {
    ...
    useEffect(() => {
        ...
        if (paymentStatus === "success" && transactionId && !verifying) {
    ...
    const verifyPayment = async (transactionId: string) => {
        if (verifying) return; // Prevent double verification
        ...
        try {
            ...
            if (response.ok) {
                ...
                setShowErrorAlert(true);
                setErrorMessage(error.detail || "Payment verification failed. Please contact support.");
                setLoading(false);
            }
        } catch (error) {
            console.error("Error verifying payment:", error);
            setShowErrorAlert(true);
            setErrorMessage("Unable to verify payment. Please contact support with your transaction ID.");
            setLoading(false);
        } finally {
            setVerifying(false);
        }
    ...
    return (
        <div>
            ...
            {/* Error Alert */}
            {showErrorAlert && (
                <div className="bg-red-500/10 border border-red-500 rounded-xl p-4 mb-6 flex items-start">
                    ...
                    <div>
                        <h3 className="text-red-500 font-semibold">Payment Failed</h3>
